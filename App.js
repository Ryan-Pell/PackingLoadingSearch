...
class SearchPart extends Component {
  ...
  async BarCodeScanned({ type, data, bounds, cornerPoints }) {
    ...
    if (!scanned && (bounds === undefined || (bounds.origin.x >= MaskDetails.x && bounds.origin.y >= MaskDetails.y && Math.floor(bounds.size.height + bounds.origin.y) <= Math.floor(MaskDetails.height + MaskDetails.y) && Math.floor(bounds.size.width + bounds.origin.x) <= Math.floor(MaskDetails.width + MaskDetails.x)))) {
      Vibration.vibrate();
